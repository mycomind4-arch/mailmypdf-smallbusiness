...
export const onRequestPost = async ({ request, env }: { request: Request; env: Env }): Promise<Response> => {
  try {
    ...
    const response = await supabaseRest(env, `mailing_intents?id=eq.${encodeURIComponent(mailingIntentId)}&requested_by=eq.${encodeURIComponent(user.id)}&select=id,status,workflow_id,mail_job_id,business_id,stripe_session_id,trigger_response`);
    ...
    if (intent.status === "queued" || intent.status === "processing" || intent.status === "mailed") return json({ success: true, status: intent.status, mailingIntentId, idempotent: true });
    ...
    const triggerResponse = await fetch(`${(env.TRIGGER_API_URL || "https://api.trigger.dev").replace(/\/$/, "")}/api/v1/tasks/${encodeURIComponent(String(intent.workflow_id))}/trigger`, {
      method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${env.TRIGGER_SECRET_KEY}` },
    ...
    return json({ success: true, status: "queued", mailingIntentId, idempotent: false });
  } catch (error) { if (error instanceof Response) return error; return json({ error: error instanceof Error ? error.message : "Unable to queue paid execution." }, 502); }
};
